...
#' @description Creates a raster by any 3 component from Landsat bands. If any channel left, a black and white raster is generated
#'
...
ToRGB<-function(red.component, green.component = NULL, blue.component = NULL){
     ...
     # do a black and white image if any channels left to the list
     if(is.null(blue.component) | is.null(green.component)){
          the.color <- round((SGDF$blue + SGDF$green +SGDF$red) / 3)
          the.color[the.color < 0] <- 0
          the.color[the.color > 255] <- 255
          SGDF$blue <- the.color
          SGDF$green <- the.color
          SGDF$red <- the.color
     }
